...
const isFacet = (json: any): json is Facet => {
  ...
  if (
    !json.counts.every(
      (countsObj: any) => "count" in countsObj && "value" in countsObj
    )
  )
    return false;
...
const isHit = (json: any): json is Hit => {
  ...
  if (
    !json.highlights.every(
      (highlight: any) => "field" in highlight && "snippet" in highlight
    )
  )
    return false;
...
export function isMeetingDocumentMetadata(
  json: any
): json is MeetingDocumentMetadata<boolean> {
...
export function isCancelled(
  target: any
): target is MeetingDocumentMetadata<true> {
